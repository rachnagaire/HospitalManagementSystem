...
def insert_patient(resource, cursor):
    ...
    cursor.execute("""
        INSERT INTO Patient (identifier, First_name, Last_name,gender,birthDate,marital_status)
        VALUES (%s, %s, %s, %s, %s,%s)
    ...
    """, ( patient_id, patient_First, patient_Last,patient_gender,patient_birthDate,patient_Marital))
...
def insert_claim(resource, cursor):
    try:
        ...
        secondary_insurance = resource.get("insurance", [{}])[1].get("coverage", {}).get("display", '') if len(resource.get("insurance", [])) > 1 else ''
...
#process each resource in the JSON bundle
def process_fhir_resource(fhir_data,cursor):
    #processes each resource in the FHIR JSON data and inserts them into the appropriate MySQL tables based on resource type.
...
def read_json_files_from_directory(directory_path):
    return [os.path.join(directory_path,f) for f in os.listdir(directory_path) if f.endswith('.json')]


def insert_all_data_to_mysql(data):
    connection = connect_to_db()
    if connection:
        cursor = connection.cursor()
        fhir_data = load_json_file('JSON_Data/Ada662_Sari509_Balistreri607_dbc4a3f7-9c69-4435-3ce3-4e1988ab6b91.json')
        process_fhir_resource(fhir_data, cursor)
...
if __name__ == '__main2__':
    ...
    if connection:
        ...
        cursor.close()
        connection.close()
        print("Data processing complete and connection closed")

if __name__ == '__main__':
    directory_path='JSON_Data'
    json_files=read_json_files_from_directory(directory_path)
    processed_count=0
    connection = connect_to_db()
    if connection:
        cursor=connection.cursor()
        for json_file in json_files:
            ...
            try:
                fhir_data=load_json_file(json_file)
                process_fhir_resource(fhir_data,cursor)
                processed_count +=1
            except Exception as e:
        ...
        cursor.close()
        connection.close()
        print(f"Data processing complete {processed_count} files processed and connection closed")
